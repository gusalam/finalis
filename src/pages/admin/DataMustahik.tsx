...
import { Plus, Trash2, Pencil, FileText, RotateCcw, Printer, FileSpreadsheet } from 'lucide-react';
...
export default function DataMustahik() {
  ...
  const fetchData = async () => {
    const [{ data: m, error: mustahikError }, { data: rt, error: rtError }] = await Promise.all([
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Total Jiwa</TableHead>
            ...
            <TableBody>
              {data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center text-muted-foreground py-10">Tidak ada data mustahik</TableCell>
                </TableRow>
              ) : data.map((item) => (
