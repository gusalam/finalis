...
-- Fix: Zakat Fitrah beras uses SUM(jumlah_beras) directly, not jumlah_jiwa * 2.5
...
AS $function$
  SELECT json_build_object(
    ...
    'total_beras_fitrah', COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d WHERE d.jenis_zakat = 'Zakat Fitrah'), 0),
    'total_beras_fidyah', COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d WHERE d.jenis_zakat = 'Fidyah'), 0),
    'total_beras', COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d WHERE d.jenis_zakat = 'Zakat Fitrah'), 0) + COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d WHERE d.jenis_zakat = 'Fidyah'), 0),
...
AS $function$
  SELECT json_build_object(
    ...
    'total_beras_fitrah', COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d JOIN transaksi_zakat t ON t.id = d.transaksi_id WHERE d.jenis_zakat = 'Zakat Fitrah' AND (_start_date IS NULL OR t.tanggal >= _start_date) AND (_end_date IS NULL OR t.tanggal <= _end_date)), 0),
    ...
    'total_beras', COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d JOIN transaksi_zakat t ON t.id = d.transaksi_id WHERE d.jenis_zakat = 'Zakat Fitrah' AND (_start_date IS NULL OR t.tanggal >= _start_date) AND (_end_date IS NULL OR t.tanggal <= _end_date)), 0) + COALESCE((SELECT SUM(d.jumlah_beras) FROM detail_zakat d JOIN transaksi_zakat t ON t.id = d.transaksi_id WHERE d.jenis_zakat = 'Fidyah' AND (_start_date IS NULL OR t.tanggal >= _start_date) AND (_end_date IS NULL OR t.tanggal <= _end_date)), 0),
