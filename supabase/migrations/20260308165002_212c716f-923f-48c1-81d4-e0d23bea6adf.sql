...
-- Update get_zakat_stats to separate beras fitrah, beras fidyah, and use jiwa-based calculation
...
AS $function$
  SELECT json_build_object(
    ...
    'total_beras_fitrah', COALESCE((SELECT SUM(jumlah_jiwa) FROM zakat WHERE jenis_zakat = 'Zakat Fitrah'), 0) * 2.5,
    'total_beras_fidyah', COALESCE((SELECT SUM(jumlah_beras) FROM zakat WHERE jenis_zakat = 'Fidyah'), 0),
    'total_beras', COALESCE((SELECT SUM(jumlah_jiwa) FROM zakat WHERE jenis_zakat = 'Zakat Fitrah'), 0) * 2.5 + COALESCE((SELECT SUM(jumlah_beras) FROM zakat WHERE jenis_zakat = 'Fidyah'), 0),
...
AS $function$
  SELECT json_build_object(
    ...
    'total_beras_fitrah', COALESCE((SELECT SUM(jumlah_jiwa) FROM zakat WHERE jenis_zakat = 'Zakat Fitrah' AND (_start_date IS NULL OR tanggal >= _start_date) AND (_end_date IS NULL OR tanggal <= _end_date)), 0) * 2.5,
    'total_beras_fidyah', COALESCE((SELECT SUM(jumlah_beras) FROM zakat WHERE jenis_zakat = 'Fidyah' AND (_start_date IS NULL OR tanggal >= _start_date) AND (_end_date IS NULL OR tanggal <= _end_date)), 0),
    'total_beras', COALESCE((SELECT SUM(jumlah_jiwa) FROM zakat WHERE jenis_zakat = 'Zakat Fitrah' AND (_start_date IS NULL OR tanggal >= _start_date) AND (_end_date IS NULL OR tanggal <= _end_date)), 0) * 2.5 + COALESCE((SELECT SUM(jumlah_beras) FROM zakat WHERE jenis_zakat = 'Fidyah' AND (_start_date IS NULL OR tanggal >= _start_date) AND (_end_date IS NULL OR tanggal <= _end_date)), 0),
